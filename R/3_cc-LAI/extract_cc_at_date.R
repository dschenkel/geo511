...
ncdf_to_mtrx <- function(filename, prod) {
	...
	#rS <- stack( mget( paste0("r", 1:dims[3]) , envir = .GlobalEnv ) )
	out <- as.array(rS)
	#print(dim(out))
	return(out)
}
	...
	if(doy>dt) {
		if(doy>length(cur_cc)) {
			...
			if((doy-dt)>length(cur_cc)) { # both start and end_date are in next year
				...
				return(mean(next_cc[start_date:end_date]))
			}
			else { # year-end problem
				end_date.next = doy - length(cur_cc)
				start_date.next = 1
...
# amount of days before PARAM date to include
#dt = 30
...
counter = 1
param.name = list("SOS","bla","blub", "EOS")
for(param in c(1,4)) {
	for(year in 1982:2011) {
		...
		for(prod in c("TEMP_FAC","MOIST_FAC","LIGHT_FAC")) {
			...
			filename.cc.prev <- paste(rootdir,"LAIre/raw_data/Global-0.5x0.5.analysis.",year-1,".nc",sep="")
			filename.cc.next <- paste(rootdir,"LAIre/raw_data/Global-0.5x0.5.analysis.",year,".nc",sep="")
